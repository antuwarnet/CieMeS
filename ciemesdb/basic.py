...
import time
import re
from ciemesdb.getsource import *
from ciemesdb.config import *

cmseek_dir = os.path.dirname(os.path.abspath(__file__)).replace('ciemesdb','')[:-1]    
total_requests = 0
cstart = time.time()
...
if access_directory == "" or not os.path.exists(access_directory):
    ...
    if os.access(ciemes_dir, os.W_OK):
        # use the parent CieMeS directory if it is writeable
        access_directory = ciemes_dir
    else:
        if ciemes_dir == os.getcwd():
            # current directory and ciemes directory are same and write access not available. show error if --batch is not used
            if not batch_mode:
                input(bold + red + "[x] " + "No write access in current directory, [ENTER to continue]" + cln)
...
def banner (txt):
    ...
    print(bold + fgreen + """
        ...
        $$    $$/ $$ |$$       |$$ | $/  $$ |$$       |$$    $$/ 
         $$$$$$/  $$/  $$$$$$$/ $$/      $$/  $$$$$$$/  $$$$$$/  
""".format(lblue, red, white))
    if txt != "":
        print(whitebg + black + bold)
        print(" [*]  " + txt + "  [*] " + cln)
    else:
        print(cln + bold + lbluebg + black + " Author: " + cln + bold + " https://instagram.com/cantixcr3w" + blackbg + white + "\n GitHub: " + cln + bold + " https://github.com/cantixcr3w \n" + cln + '\n')
    print(cln)
    return
...
USAGE:
       python3 ciemes.py (for guided scanning) OR
       python3 ciemes.py [OPTIONS] <Target Specification>
...
EXAMPLE USAGE:
      python3 ciemes.py -u example.com                           # Scan example.com
      python3 ciemes.py -l /home/user/target.txt                 # Scan the sites specified in target.txt (comma separated)
      python3 ciemes.py -u example.com --user-agent Mozilla 5.0  # Scan example.com using custom user-Agent Mozilla is 5.0 used here
      python3 ciemes.py -u example.com --random-agent            # Scan example.com using a random user-Agent
      python3 ciemes.py -v -u example.com                        # enabling verbose output while scanning example.com

    """.format(ciemes_version,red, cln, bold, GIT_URL))
    bye()
...
def bye():
    bye_dict = ["Rex4","GadizNR","Swaat Develona","Airin","Yui24","M4nj4Lit4","GirlsHaxor","AnzeL0L"]
    this_time = random.choice(bye_dict)
    print('\n' + bold + red + ' CieMeS says ~ ' + this_time + cln)
    quit()
...
def targetinp(iserr):
    ...
    if iserr != "":
        ...
        target = input("Enter target (https://cantixcr3w.com): ").lower()
    if "://" in target and "http" in target:
        ...
        return targetinp(red + "Invalid URL,correct format (https://cantixcr3w.com)")
...
def clear_log():
    ...
    if os.path.isdir(resdir):
        ...
        os.makedirs(resdir)
        success('Result directory cleared success!')
        bye()
    else:
        warning('Results directory ga ketemu bre!')
        bye()
...
def update_brute_cache():
    ...
    if not os.path.isdir(brute_dir):
        try:
            error('CieMeS could not find the bruteforce directory, Creating Brute directory')
            os.makedirs(brute_dir)
            info('Bruteforce directory created, add some modules from: https://github.com/cantixcr3w/CieMeS')
            bye()
    ...
    for f in py_files:
        if f.endswith('.py') and f != '__init__.py':
            ...
            mod_cnt = fo.read()
            if 'ciemesbruteforcemodule' in mod_cnt and 'Bruteforce module' in mod_cnt:
                n = []
    ...
    if not modules == [] and modulen != []:
        ...
        print('\n')
        result('Cache Updated! Enjoy CieMeS with new modules ;)','')
    else:
...
def update():
    ...
    get_version = getsource('https://raw.githubusercontent.com/cantixcr3w/CieMeS/master/current_version',randomua('generate'))
    if get_version[0] != '1':
        ...
        info("CieMeS Version: " + cmseek_version)
        ...
        if my_version > serv_version:
            ...
            print('\n')
            result("CieMeS is up to date, Thanks for checking update tho.. It's a good practise",'')
        else:
            print('\n')
...
def savebrute(url,adminurl,username,password):
    ...
    if url != "" and adminurl != "" and username != "" and password != "":
        ...
        brute_result = "### CieMeS Bruteforce Result\n\n\nSite: " + url + "\n\nLogin URL: " + adminurl + "\n\nUsername: " + username + "\n\nPassword: " + password
