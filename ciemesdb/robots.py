...
def check(url, ua):
    ...
    robots_source = ciemes.getsource(robots, ua)
            ...
            if ':-' in detection_key:
                ...
                if '||' in detection_array[0]:
                    ...
                    for detection_string in detection_strings:
                        if detection_string in robots_txt_content and detection_array[1] not in ciemes.ignore_cms:
                    ...
                    if detection_array[0] in robots_txt_content and detection_array[1] not in ciemes.ignore_cms:
                        if ciemes.strict_cms == [] or detection_array[1] in ciemes.strict_cms:
                            return ['1', detection_array[1]]

        t3_regex = re.search(r'Sitemap: http(.*?)\?type=', robots_txt_content)
        if t3_regex != None and 'tp3' not in ciemes.ignore_cms:
            if ciemes.strict_cms == [] or 'tp3' in ciemes.strict_cms:
                return ['1', 'tp3']

        return ['0','']
    else:
        ciemes.error('robots.txt not found or empty!')
        return ['0','']
